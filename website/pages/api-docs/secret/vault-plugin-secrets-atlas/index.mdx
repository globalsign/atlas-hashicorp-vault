---
layout: api
page_title: GlobalSign Atlas- Secrets Engines - HTTP API
sidebar_title: Atlas
description: This is the API documentation for the Vault GlobalSign Atlas secrets engine.
---

# GlobalSign Atlas Secrets Engine (API)

This is the API documentation for the Vault GlobalSign Atlas secrets engine. For general
information about the usage and operation of the GlobalSign Atlas secrets engine, please see
the [Getting Started Guide](https://github.com/globalsign/atlas-hashicorp-vault/blob/master/website/pages/docs/secrets/vault-plugin-secrets-atlas/index.mdx).

This documentation assumes the Atlas secrets engine is enabled at the `/atlas` path
...
- [Configure Atlas Authentication](#read-ca-certificate)
...
This endpoint configures the Vaults connection to your GlobalSign Atlas account.
It requires mTLS and API credentials provided by your account manager. These credentials will be
persisted in your vault cluster so you will only need to provide them once.
...
structure and cannot be read by the Vault CLI; use `/atlas/cert` for that.
...
be read by the Vault CLI; use `/atlas/cert` for that. If `/pem` is added to the
...
revocation will update your Atlas CRL.
...
-> NOTICE: Roles are enforced within the Vault, your Atlas Instance roles that
    are more permissive than your Atlas Instance may still result in certificate
...
- `generate_lease` `(bool: false)` – Specifies if certificates issued/signed
  ...
  with leases. It can also be done using the `atas/revoke` endpoint. However,
  when lease generation is disabled, invoking `atas/revoke` would be the only way
...
Atlas root CA need be in a client's trust store.
...
This endpoint allows tidying up the storage backend and by removing

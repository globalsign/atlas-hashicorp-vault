...
func (b *Backend) pathFetchRead(ctx context.Context, req *logical.Request, data *framework.FieldData) (response *logical.Response, retErr error) {
	...
	atlasClient, retErr := b.GetAtlasClient(ctx, req.Storage)
	if funcErr != nil {
		return nil, retErr
	}
	...
	if serial == "ca_chain" {
		var certStr string
		var caChain []*certutil.CertBlock
		caChain, retErr = trustChainToBlocks(ctx, atlasClient)
		if retErr != nil {
			return nil, retErr
		}
	...
	if serial == "ca" {
		...
		certificate = pem.EncodeToMemory(&pem.Block{
			Type: pemType,
			// Assumption Atlas returns leaf first
			Bytes: caChain[0].Bytes,
		})
		goto reply
	}
...
Fetch a CA, CA Chain, or non-revoked certificate.
`
